...
const LoginPage = ({ onSignIn, onGoToRegister }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        /* 🔄 CORRECTION: Committing credentials to session storage instead of local storage */
        sessionStorage.setItem('sewa_user', JSON.stringify(data.user));
        sessionStorage.setItem('sewa_user_id', data.user.id || data.user.userId);
