...
const YearlyMonthChart = ({ viewMode = 'monthly' }) => {
  const currentYear = new Date().getFullYear();
  ...
  const [countryDirection, setCountryDirection] = useState('destination'); // 🆕 'destination' | 'origin'
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const chartRef = useRef(null);        // 🆕 points at the chart container
  ...
  // Monthly/yearly data fetch (unchanged, only runs in monthly mode)
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      try {
        ...
        const response = await fetch(`https://sewaro-backend.onrender.com/api/admin/analytics/volume-matrix?startYear=${startYear}&endYear=${endYear}`);
        if (!response.ok) throw new Error('Database analytics engine failure computing yearly matrix trace.');
  ...
  // Country-wise data fetch (new, only runs in country mode)
  useEffect(() => {
    ...
    const fetchCountryData = async () => {
      try {
        ...
        const response = await fetch(`https://sewaro-backend.onrender.com/api/admin/analytics/country-matrix?startYear=${startYear}&endYear=${endYear}&direction=${countryDirection}`);
        if (!response.ok) throw new Error('Database analytics engine failure computing country matrix trace.');

        const result = await response.json();
        if (result.success) {
          setCountryData(result.data.countryMatrix);
        } else {
          throw new Error(result.error);
        }
      } catch (err) {
    ...
    fetchCountryData();
  }, [startYear, endYear, viewMode, countryDirection]);

  // 🆕 Scroll to chart whenever the user switches view/direction (skips initial page load)
  useEffect(() => {
  ...
  }, [viewMode, countryDirection]);
    useEffect(() => {
    if (!loading && pendingScroll.current && chartRef.current) {
  ...
  const rechartsAdaptedData = useMemo(() => {
    ...
    return monthNames.map(month => {
      const adaptedEntry = { name: month };
      for (let year = configuredStart; year <= configuredEnd; year++) {
        adaptedEntry[year] = chartMatrix[year]?.[month] || 0;
      }
      return adaptedEntry;
    });
  }, [chartMatrix, startYear, endYear]);
  ...
  // ── Country-wise view ──
  if (viewMode === 'country') {
    if (!countryData || countryData.length === 0) return <div className="chart-empty">No country data available for this range.</div>;

    return (
      <div className="yearly-chart-module" ref={chartRef}>
         <div className="chart-controls no-print" style={{ marginBottom: '10px' }}>
          <label>Show:
            <select value={countryDirection} onChange={(e) => setCountryDirection(e.target.value)}>
              <option value="destination">Destination (where shipments go)</option>
              <option value="origin">Origin (where shipments come from)</option>
            </select>
          </label>
        </div>
        ...
        <div className="chart-wrapper">
          <ResponsiveContainer width="100%" height={380}>
            ...
            >
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
              <XAxis dataKey="country" tick={{ fontSize: 13, fill: '#64748b' }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 13, fill: '#64748b' }} axisLine={false} tickLine={false} label={{ value: 'Tracking Volume', angle: -90, position: 'insideLeft', offset: 18, fill: '#1e293b', fontSize: 12 }} />
              <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }} cursor={{ fill: 'rgba(241, 245, 249, 0.5)' }} />
              <Bar dataKey="shipments" fill={colors[0]} radius={[4, 4, 0, 0]} name="Shipments by Country">
                <LabelList dataKey="shipments" position="top" fill="#1e293b" fontSize={11} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
  ...
  return (
    <div className="yearly-chart-module" ref={chartRef}>
      ...
      <div className="chart-wrapper">
        <ResponsiveContainer width="100%" height={380}>
          <BarChart data={rechartsAdaptedData} margin={{ top: 20, right: 30, left: -10, bottom: 5 }} barGap={4}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
            <XAxis dataKey="name" tick={{ fontSize: 13, fill: '#64748b' }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 13, fill: '#64748b' }} axisLine={false} tickLine={false} label={{ value: 'Tracking Volume', angle: -90, position: 'insideLeft', offset: 18, fill: '#1e293b', fontSize: 12 }} />
            <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)' }} cursor={{ fill: 'rgba(241, 245, 249, 0.5)' }} />
            {isMultiYear && <Legend wrapperStyle={{ fontSize: '13px', paddingTop: '15px' }} iconType="rect" />}
            {yearsActive.map((year, index) => (
              <Bar key={year} dataKey={year} fill={colors[index % colors.length]} radius={[4, 4, 0, 0]} name={isMultiYear ? `Shipments: ${year}` : 'Active Total Tracking Matrix'}>
                {!isMultiYear && <LabelList dataKey={year} position="top" fill="#1e293b" fontSize={11} />}
              </Bar>
            ))}
          </BarChart>
